...
public class CassandraTestRepositoryInitializer implements TestRepositoryInitializer {
    ...
    public void setUp() {
        ...
        session.execute("CREATE TABLE IF NOT EXISTS gravitee.apis (id text PRIMARY KEY, name text, description text, version text, definition text, deployed_at timestamp, created_at timestamp, updated_at timestamp, visibility text, lifecycle_state text, picture text, group text, views set<text>, labels list<text>);");
        session.execute("CREATE TABLE IF NOT EXISTS gravitee.applications (id text PRIMARY KEY, name text, description text, type text, created_at timestamp, updated_at timestamp, group text, status text);");
